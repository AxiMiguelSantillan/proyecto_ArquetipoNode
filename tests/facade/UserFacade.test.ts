process.env.NODE_ENV = 'test'

import { expect } from "chai";
import UserFacade from '../../src/facade/User/facade';
import { db } from '../../src/config/connection/database';
import User from "../../src/models/User.model";
import * as Kafka from "../../src/config/stream/kafka";
import { UserTo } from "../../src/to/UserTo";
import { ParametersError } from "../../src/config/error";

describe('UserFacade Test', () => {

    before('Init', async() => {
        await db.sync({ force: true});
        User.create({
        id: 1,
        name: 'Juan',
        email: 'test@axity.com',
        createdAt: '2020-01-01',
        updatedAt: '2020-01-01'
        });
    });
  
    describe('FindAll', () => {
        it('should return one user', async () => {
            const User: any[] = await UserFacade.findAll();
            expect(1).equal(User.length);
        });
    });

    describe('Create', () => {
        it('should create one user', async () => {
            let userTo: UserTo = {
                name: "Juan",
                email: "Juan@axity.com"
            }
            const user: UserTo = await UserFacade.create(userTo);
            expect(user.name).equal("Juan");
        });
    });
 
    describe('Create Error User Exist', () => {
        it('should retur error', async () => {
            let userTo: UserTo = {
                name: "Juan",
                email: "test@axity.com"
            }
            try {
                await UserFacade.create(userTo);
            } catch (error) {
                expect(error).instanceOf(ParametersError)
            }
        });
    });
    describe('Create Error atributes required', () => {
        it('should retur error -> attributes required', async () => {
            let userTo: UserTo = {
                name: "alberto"
            }
            try {
                await UserFacade.create(userTo);
            } catch (error: any) {
                expect(error).instanceOf(ParametersError)
                expect(error.message).equal('El atributo email es requerido')
            }
        });
    });

    describe('Create Error atributes required', () => {
        it('should retur error -> attributes required', async () => {
            let userTo: UserTo = {
                email: "alberto@axity.com"
            }
            try {
                await UserFacade.create(userTo);
            } catch (error: any) {
                expect(error).instanceOf(ParametersError)
                expect(error.message).equal('El atributo name es requerido')
            }
        });
    });

    describe('Create Error all atributes required', () => {
        it('should retur error -> attributes required', async () => {
            let userTo: UserTo = {
            }
            try {
                await UserFacade.create(userTo);
            } catch (error: any) {
                expect(error).instanceOf(ParametersError)
                expect(error.message).equal('El atributo name es requerido' || 'El atributo email es requerido')
            }
        });
    });
    
    describe('Update', () => {
        it('should update one user', async () => {
            let userTo: UserTo = {
                name: "miguel",
                email: "miguel@axity.com"
            }
            try {
                await UserFacade.updateOne(1, userTo);
            } catch (error: any) {
                expect(error).instanceOf(ParametersError)
            }
        });
    });

    describe('Update Error, Id no exist', () => {
        it('should retur error', async () => {
            let userTo: UserTo = {
                name: "miguel",
                email: "miguel@axity.com"
            }
            try {
                await UserFacade.updateOne(99, userTo);
            } catch (error: any) {
                expect(error.message).equal('No es posible actualizar este usuario')
            }
        });
    });

    describe('Update Error, atributes required', () => {
        it('should retur error', async () => {
            let userTo: UserTo = {
            }
            try {
                await UserFacade.updateOne(1, userTo);
            } catch (error: any) {
                expect(error.message).equal('No es posible actualizar este usuario')
            }
        });
    });

    describe('Delete', () => {
        it('should delete one user', async () => {
            try {
                await UserFacade.deleteOne(1);
            } catch (error: any) {
                expect(error).instanceOf(ParametersError)
            }
        });
    });

    describe('Delete Error, Id no exist', () => {
        it('should retur error', async () => {
            try {
                await UserFacade.deleteOne(99);
            } catch (error: any) {
                expect(error.message).equal('No es posible eliminar este usuario')
            }
        });
    });

  

});